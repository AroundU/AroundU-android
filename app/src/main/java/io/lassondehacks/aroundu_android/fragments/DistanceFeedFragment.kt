...
class DistanceFeedFragment(var ctx: Context)  : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater?, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view = inflater?.inflate(R.layout.fragment_distance_feed, container, false)
        val swiperefresh = (view?.findViewById(R.id.swiperefresh3) as SwipeRefreshLayout)
        (view?.findViewById(R.id.swiperefresh3) as SwipeRefreshLayout).setOnRefreshListener {
            val thread = Thread {
                refreshFeed()
    ...
    fun refreshFeed() {
        ...
        FeedService.getDistanceFeedPosts(
    ...
    fun populatePostCards() {
        (view?.findViewById(R.id.cards_container3) as LinearLayout).removeAllViews()
